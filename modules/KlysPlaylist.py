...
def processPlaylists(pdir, cdir):
    ...
    with open('overall.m3u', 'w') as opl:
        ...
        for (p, _, f) in os.walk(pdir):
            ...
            if music:
                with open(os.path.join(cdir, 'Individual', os.path.split(p)[1]) + ".m3u", "w") as pl:
                    for m in music:
                        pl.write(m + "\n")
                        opl.write(m + "\n")
                        pb.update()
