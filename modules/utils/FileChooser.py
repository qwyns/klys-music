...
def PrintDirectories(i):
    dirlist = ['. (Choose)', '.. (Parent)', *[f.name for f in os.scandir(i) if f.is_dir()]]
    longest_name_len = max([len(_) for _ in dirlist])
    entries_per_row = math.ceil(140/(longest_name_len + 4))
    print ('\n'.join([''.join((*map(lambda x: x.ljust(longest_name_len + 4), dirlist[_:_+entries_per_row]), )) for _ in range(0, len(dirlist), entries_per_row)]))
